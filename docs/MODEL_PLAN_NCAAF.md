...
| 8 | Total accuracy: five recalibrations of the same state forecasts (`atlas/research/ncaaf_total_accuracy.py`) | **measured; none adopted.** Adjusted efficiency sums, the pace defences allow, recency-weighted calibration, five training seasons and an in-season running level, 2021–25 against the total as it stands (CRPS 9.133; market 8.842). The efficiency sums clear the bar as written (9.124, three seasons of five) on a paired change a third of its own standard error; recency weighting is worse beyond noise (t +2.4); the rest are noise. The bar lacked a test of noise and future bars carry one. What the total lacks is information the state does not have - the quarterback (factor 9, −3.2 points on a change) first (`reports/ncaaf_total_accuracy.md`) |
| 9 | The quarterback state (`atlas/models/ncaaf_qb.py`), from ESPN box scores: a state per starter inside the season, forecast with the previous quarterback of record | **measured; fails, and why is measured.** Margin paired CRPS +0.0055 (t +1.24), total +0.0035 (t +1.49); the tuning kept the quarterback prior as small as its grid allowed. The state misses a team by −2.46 points in the game a new quarterback starts and by −0.09 in his next: the effect is in the change game, which the record shows only afterwards, and the team state absorbs it from that game. Only a pre-kickoff starter source would help, and Atlas has none for college. Not wired (`reports/ncaaf_qb.md`) |
