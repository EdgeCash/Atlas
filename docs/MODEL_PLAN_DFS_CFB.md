...
Status: **steps 0-5 done** (see §5). The NFL DFS model (`docs/MODEL_PLAN_DFS.md`)
...
| 5 | College slates on the owner page (`atlas/dfs/cfb_participation.py`, `atlas/dfs/cfb_slate.py`) | decrypts; every college slate listed — **done.** Who records a stat: a team's candidates are the players with a stat in any of its previous eight games, and a walk-forward classifier on their recent appearances, career, scoring, position and point in the season scores a Brier of 0.133 against 0.174 for the share of the last four games, within 2.3 points of the observed rate in every tenth (`reports/dfs_cfb_participation.md`). A priced player with no record for his team gets a flat 30%, stated as a guess. Live, each DraftKings player is matched to ESPN's record by name and team, then name and position (a transfer keeps his history; one record never goes to two players), each game to ESPN's upcoming one by its two teams, and the game read from the line Atlas captures for its cards. The projection is the chance he plays times the model's, the range the mixture of the two. Every college Classic (five lineups, the SUPERFLEX included) and Showdown (five) is on the owner page beside the NFL's, labelled by sport, with college's player table under the NFL's. A college failure is logged by type and costs the NFL nothing; on the first live run, 15 slates and 2,000 priced players, 855 of them with no record (nearly all minimum-salary backups) |
| 6 | Public college projections | a decision after the live record, not a step. **The record is kept** (`atlas/dfs/cfb_record.py`): every priced college player with a game, one row per (game, DraftKings player), rewritten by each heavy refresh until his kickoff and never after, graded against ESPN's box score (zero for a player who recorded nothing; a player with no record when projected found by name and team). It is the owner's, so it is sealed with the owner key, compressed, one file a week in `tracking/dfs_cfb_record/`, committed with the tracking store as its backup; a key that cannot open it leaves it untouched. The owner page shows it: average miss, how often the 80% range held and rank correlation, for each team's regulars and for every priced player |
