...
Status: **steps 0-8 built** (see §7): the public area and every NFL salary format on the owner page. Scope v1: DraftKings NFL Classic,
...
| 3 | Projection model v1, team-anchored (`atlas/dfs/environment.py`, `atlas/dfs/context.py`, `atlas/dfs/model.py`; `make dfs-model`) | **done — gate passes** (`reports/dfs_projections.md`). 2015-21 regulars, CRPS against the baseline: QB 4.58 vs 4.75, RB 4.14 vs 4.23, WR 4.16 vs 4.24, TE 3.58 vs 3.61, DST 3.23 vs 3.35; rank against salary: QB 0.341 vs 0.330, RB 0.523 vs 0.506, WR 0.493 vs 0.484, TE 0.420 vs 0.403, DST 0.284 vs 0.243 (with step 4's defenses). 2022-25 holds the same margins over the baseline. Atlas-only, the model fell short of salary's ranking at QB (0.315) and DST (0.241); the gap was the game environment - the line's team totals rank a team's players better than Atlas's projected points - so the owner added them as an input (24 September 2026). The report keeps the Atlas-only result |
| 4 | Defenses and ranges (`atlas/dfs/defense.py`, `atlas/dfs/ranges.py`; in `make dfs-model`) | 80% ranges cover 76-84% — **done, gate passes.** Ranges are each position's 10th and 90th percentile of the model's own out-of-sample misses, as lines in the projection (fitted separately: DraftKings points are lopsided). Regulars 2015-21: DST 80.1%, QB 81.0%, RB 80.0%, TE 81.2%, WR 79.9% (80.2% overall); 2022-25: 80.1-83.1%; every season 2015-25 between 78.9% and 82.0%. Defenses: expected sacks, takeaways and return touchdowns (Poisson rates on form, the opposing offense and both teams' projected points), league rates for the rare events, and the points-allowed bonus averaged over the opponent's score spread; the published projection averages this with the player model. Rank among defenses 2015-21: 0.284 (salary 0.243); 2022-25: 0.297 |
| 5 | Optimizer and DraftKings CSV (`atlas/dfs/optimizer.py`, `atlas/dfs/slate.py`, `atlas/dfs/participation.py`, `atlas/dfs/backtest.py`; `make dfs-slate`, `make dfs-lineups`) | constraints always hold; brute-force check — **done.** SciPy `milp` matches exhaustive enumeration on 18 slates (plain, stacked under a tight cap, stacked with a bring-back and a team limit); every lineup passes an independent rule check; several lineups at once respect uniqueness and exposure. Backtest, 119 Sunday main slates 2015-21 (`reports/dfs_lineups.md`): the model's lineup scored 136.8 on average, the baseline's 127.7, salary's 123.4 (model higher on 71% of slates against salary, +13.4 ± 3.0). Live: the pool is matched to nflverse ids (name and team, then position, then the player list), each projection multiplied by the chance the player records a stat (walk-forward Brier 0.121 against 0.173 for position and depth alone, calibrated within 4 points in every tenth), and the upload file carries DraftKings' draftable ids, now in the capture. Output under `data/dfs/`, not committed. First live run, week 3 Main: 662 players, 7 with no history |
| 6 | Owner page, encrypted, in the heavy run (`atlas/dfs/owner.py`, `atlas/site/assets/owner.js`, `dfs/owner.html`) | decrypts on the iPad with the passphrase; nothing readable published — **done, gate passes: the first heavy run with the secret went green and the page opened on the owner's iPad (24 September 2026).** The heavy run's `dfs` step (at first `dfs-owner`) rebuilds the DFS tables, projects the Main slate, builds ten lineups and seals them (PBKDF2-SHA256 at 600,000 iterations, AES-256-GCM, fresh salt and nonce) before anything reaches `site/`; the page holds only ciphertext and opens it in the browser with WebCrypto. Checked: Python's box opens in Node's WebCrypto and in Chromium (0.9 s, iPad and phone widths), a wrong passphrase is refused, the download is DraftKings' upload file, no player name or draftable id appears in the built page, the page is `noindex` and outside the sitemap, the audit passes. The step never fails the run (no secret: nothing built; an error: its type only). The ciphertext has its own Actions cache entry so the hourly poll republishes it; readable output stays in `data/dfs/`, never cached, committed or published. Cached play-by-play missing the step 1 fields is now fetched again automatically |
| 7 | Public DFS area: projections with ranges and the record; audit rules; About and FAQ reworded (`dfs.html`, `atlas/dfs/record.py`, `atlas/site/assets/dfs.js`) | audit passes — **built; the audit passes.** Every priced player on the upcoming Main slate by projection, with the 10th-90th range and the chance he plays; a position filter that only shows and hides rows; players under a one-in-four chance of playing folded away. No lineup, nothing featured. The record: the walk-forward history 2015-2025 by position (`reports/dfs_record.json`: miss against recent form, ranking against salary's, range coverage) and the live record - each slate's projections kept in `tracking/dfs_projections.csv` as they stood before the first kickoff (a slate that has started is never rewritten) and graded against the points scored, a non-player at zero. Every DFS page carries the adults-only note, DraftKings' responsible-gaming link and 1-800-GAMBLER; the audit fails a DFS page without them, or with promise words (optimal, winning lineups, guaranteed, profit). About and the FAQ say the no-selections promise is the cards', and why the DFS model reads the market when the cards do not. The menu gains DFS: two rows of four on a phone, and the sticky filter bar now follows the menu's measured height |

| 8 | Every NFL salary format on the owner page: all Classic slates, Showdown Captain Mode and Tiers (`atlas/sources/draftkings.py`, `atlas/dfs/kicker.py`, `atlas/dfs/optimizer.py`, `atlas/dfs/slate.py`; `make dfs-kickers`) | kicker scoring matches DraftKings'; kicker model beats its baseline's CRPS; ranges cover 76-84%; every format's optimizer matches brute force — **done, gates pass.** The capture keeps Classic (contest type 21), Showdown (96: each player's Captain salary and draftable id beside his FLEX ones) and Tiers (51: the tier from the roster slot); Snake, Best Ball, the in-game Showdowns, Madden and Single Stat are not salary lineups and are not kept. Kickers: DraftKings' scoring (field goal 0-39 yards 3, 40-49 yards 4, 50+ 5, extra point 1, no penalty for a miss) matches its own points per game for all 28 kickers in its 2026 Showdown pools; the model (recent scoring corrected by both teams' projected points from Atlas and the line, the wind and recent attempts) beats the baseline's CRPS on 2015-2025 (2.480 against 2.508), and its ranges cover 80.0% (`reports/dfs_kickers.md`). The optimizer gains Showdown (a Captain at 1.5 times points and his own salary, five FLEX, both teams) and Tiers (one per tier), both checked against brute force, and DraftKings' minimums it lacked: Classic from at least two games, Tiers too. Every upcoming slate is projected in one pass and built: ten lineups for the Main slate, five for every other Classic and Showdown slate, three for Tiers; the owner page picks a slate and downloads its upload file. First run, 24 September: 22 slates (6 Classic, 14 Showdown, 2 Tiers). The public page stays the Main slate. A player's name is marked in the page so the audit does not read a quarterback called Lock as vocabulary |

**The first heavy run with step 6** re-fetches every cached play-by-play
season that predates the fields step 1 added (who scored, who recovered,
blocked punts) - automatic, a few minutes, once - and RotoGuru's archive
(about three minutes, once); both are then in the cache.
...
- Add the repository secret `ATLAS_OWNER_KEY`: Settings → Secrets and
  variables → Actions → New repository secret. A long passphrase - five or
  more unrelated words - since the page's protection is exactly its
  strength. Then run site heavy; the page is at `dfs/owner.html` (not
  linked from the site - bookmark it). Safari offers to save the passphrase
  in the iPad's keychain.
